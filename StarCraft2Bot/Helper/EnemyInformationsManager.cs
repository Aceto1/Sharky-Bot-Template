...
namespace StarCraft2Bot.Helper
{
    ...
    public class EnemyInformationsManager
	{
		...
		public EnemyInformationsManager(UnitCountService UnitCountService, MapDataService MapDataService, ActiveUnitData ActiveUnitData,EnemyUnitMemoryService enemyUnitMemoryService,SharkyUnitData sharkyUnitData,FrameToTimeConverter frameToTimeConverter, MapMemoryService mapMemoryService)
		{
		...
		public long GetCurrentEnemyUnitMineralCost()
		{
			...
			{
				minerals += SharkyUnitData.UnitData[unitType].MineralCost * EnemyUnitMemoryService.CurrentTotalUnits[unitType];
            }
        ...
        public long GetCurrentEnemyUnitVespeneCost()
        {
            ...
            foreach (var unitType in EnemyUnitMemoryService.CurrentTotalUnits.Keys)
            {
                minerals += SharkyUnitData.UnitData[unitType].VespeneCost * EnemyUnitMemoryService.CurrentTotalUnits[unitType];
            }
		...
		public Tuple<float,float> GetApproximatedProducedEnemyMinerals(ResponseObservation observation)
		{
			...
			{
				var updateFrames = EnemyUnitMemoryService.LastTotalUnits[UnitTypes.TERRAN_SCV].Keys.OrderBy(u => u);
				...
				for (int i = 0; i < updateFrames.Count();i++)
				{
                    ...
                    int knownSCVCount = EnemyUnitMemoryService.LastTotalUnits[UnitTypes.TERRAN_SCV][currentUpdateFrame];
					...
					TimeSpan timeSpan = FrameToTimeConverter.GetTime((int)nextFrame) - FrameToTimeConverter.GetTime((int)currentUpdateFrame);
                    ...
                    minimumMinerals += (float) timeSpan.TotalMinutes * knownSCVCount * mineralsPerMinute;
			...
			foreach (UnitTypes unit in EnemyUnitMemoryService.CurrentTotalUnits.Keys) {
				if (unit != UnitTypes.TERRAN_SCV) {
					if (SharkyUnitData.UnitData[unit].Weapons.Count > 0) {
						...
						totalMinerals += SharkyUnitData.UnitData[unit].MineralCost * EnemyUnitMemoryService.CurrentTotalUnits[unit];
					}
				}
			}
		...
		public float GetVisibleAreaPercentage()
		{
            ...
            for (var x = 0; x < MapDataService.MapData.MapWidth; x++)
            {
                for (var y = 0; y < MapDataService.MapData.MapHeight; y++)
                {
					...
					{
						percentage += 100f /  ( MapDataService.MapData.MapWidth * MapDataService.MapData.MapHeight * 1f);
                    }
                }
            }
